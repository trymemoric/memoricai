...
impl Connector for OneDrive {
    ...
    async fn import(&self, ctx: &ImportCtx<'_>) -> Result<SyncStats> {
        ...
        // BFS over folders: OneDrive lists one folder level at a time, so recurse into
        // every subfolder rather than only the drive root.
        let mut folders =
            vec!["https://graph.microsoft.com/v1.0/me/drive/root/children?$top=100".to_string()];

        'outer: while let Some(folder_url) = folders.pop() {
            let mut next_url = Some(folder_url);
            while let Some(url) = next_url.take() {
                ...
                for item in list["value"].as_array().unwrap_or(&empty) {
                    ...
                    if item["folder"].is_object() {
                        if !id.is_empty() {
                            folders.push(format!(
                                "https://graph.microsoft.com/v1.0/me/drive/items/{id}/children?$top=100"
                            ));
                        }
                        continue;
                    }
                    ...
                    // Download raw bytes and extract via the binary/media extractor, instead
                    // of decoding arbitrary Office/binary files as lossy UTF-8 text.
